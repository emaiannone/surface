...
public class RunningModeFactory {
    ...
    public static RunningMode newRunningMode(String target, Path workDirPath, RunResultsExporter runResultsExporter,
        ...
        if (Utils.isPathToLocalDirectory(path)) {
            return new LocalDirectoryRunningMode(path, workDirPath, runResultsExporter, metricsManager, filesRegex, includeTests);
        }
        if (Utils.isPathToGitDirectory(path)) {
            return new LocalGitRunningMode(path, workDirPath, runResultsExporter, metricsManager, revisionSelector, filesRegex, excludeWorkTree, includeTests);
        }
        ...
        if (Utils.isPathToYamlFile(path)) {
            return new FlexibleRunningMode(path, workDirPath, runResultsExporter, metricsManager, revisionSelector, filesRegex, includeTests, excludeWorkTree);
        }
